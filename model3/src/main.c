...
int main(int argc, char *argv[])
 {
	...
	//-------------------------------------------------
    {
		...
		for(i=0; i<iNumSamples/BLOCK_SIZE; i++)
		{
			...
			if(enableFlag)
			{
				count1=cl_get_cycle_count(); // insturction count before processing

				processing(sampleBuffer, sampleBuffer); //processing function

				count2=cl_get_cycle_count();// insturction count after processing
				spent_cycles=count2-count1; //no of instructions in processing
				printf("%llu\n",spent_cycles);
			}
